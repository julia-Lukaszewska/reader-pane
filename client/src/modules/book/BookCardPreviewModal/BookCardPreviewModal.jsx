...
export default function BookCardPreviewModal() {
  ...
  // 1) Get all necessary data and hooks at the top level
  ...
  // 2) Define callbacks and effects
  ...
  // Ensure modal does not persist after route changes (component unmount)
  useEffect(() => () => {
    dispatch(clearPreviewBook())
    dispatch(resetForm())
  }, [dispatch])

  // 3) Conditional rendering check (early return)
  if (!previewBookId) {
    return null
  }
