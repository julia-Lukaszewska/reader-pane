...
import { useManageBookActions } from '@/hooks' 

const ImportFromWolneLektury = ({ onBack }) => {
  const [books, setBooks] = useState([])  
  const [loadingBookId, setLoadingBookId] = useState(null)  
  const { addBookToLibrary } = useManageBookActions() 

  useEffect(() => {
    fetch('https://wolnelektury.pl/api/books/')
      .then((response) => response.json())
      .then((data) => setBooks(data))
      .catch((error) => console.error('Error fetching Wolne Lektury:', error))  
  }, [])
